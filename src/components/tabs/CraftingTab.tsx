...
export default function CraftingTab() {
  ...
  const [craftingAnimation, setCraftingAnimation] = useState<
    "idle" | "crafting" | "success" | "fail"
  >("idle");
  ...
  const canCraftRecipe = (recipe: any) => {
    ...
    if (
      recipe.resultItem.classRestriction &&
      character?.class !== recipe.resultItem.classRestriction
    ) {
      return false;
    }
    return true;
  ...
  const availableRarities = useMemo<string[]>(() => {
    ...
    const rarities = new Set<string>(
      recipes.map((r: any) => r.resultItem.rarity as string)
    );
  ...
  // Tier badge styling for Fuse tab
  const getTierBadgeClasses = (tier: number, isFusable: boolean) => {
    const activeBase =
      tier === 0
        ? "bg-stone-800 border-stone-500 text-gray-200"
        : tier === 1
    ...
    const inactive = "bg-stone-900 border-stone-700 text-gray-500";

    return `px-1 py-0.5 border text-[10px] ${
      isFusable ? activeBase : inactive
    }`;
  };
    ...
    <div className="p-3 pb-20">
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      <div className="relative w-12 h-12 bg-stone-900 rounded flex items-center justify-center">
                        {getItemImage(
                          recipe.resultItem.spriteId,
                          recipe.resultItem.type
                        ) ? (
                          <img
                            src={
                              getItemImage(
                        ...
                        {recipe.levelRequirement > 1 && (
                          <p
                            className={`text-xs font-bold mt-1 ${
                              character &&
                              character.level >= recipe.levelRequirement
                                ? "text-green-400"
                                : "text-amber-400"
                            }`}
                          >
                    ...
                    {recipe.resultItem.attackBonus > 0 && (
                      <div className="text-xs text-orange-400">
                        ATK +{recipe.resultItem.attackBonus}
                      </div>
                    )}
                    {recipe.resultItem.defenseBonus > 0 && (
                      <div className="text-xs text-blue-400">
                        DEF +{recipe.resultItem.defenseBonus}
                      </div>
                    )}
                    {recipe.resultItem.healthBonus > 0 && (
                      <div className="text-xs text-red-400">
                        HP +{recipe.resultItem.healthBonus}
                      </div>
                    )}
                  </div>
                );
                  ...
                  >
                    <div className="flex flex-col items-center text-center gap-2">
                      ...
                      <div className="w-full">
                        <h3
                          className={`font-bold text-xs ${getRarityColor(
                            recipe.resultItem.rarity
                          )} truncate`}
                        >
                          {recipe.resultItem.name}
              ...
              <div className="bg-stone-800 rounded-lg border-4 border-amber-600 p-6 max-w-md w-full">
                ...
                <div className="flex justify-center mb-4">
                  <img
                    ...
                    alt="Anvil"
                    className={`w-20 h-20 ${
                      craftingAnimation === "crafting" ? "animate-bounce" : ""
                    }`}
                    style={{ imageRendering: "pixelated" }}
                  />
                </div>
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className="relative w-16 h-16 bg-stone-900 rounded flex items-center justify-center">
                    {getItemImage(
                      selectedRecipe.resultItem.spriteId,
                      selectedRecipe.resultItem.type
                    ) ? (
                      <img
                        src={
                          getItemImage(
                ...
                <div className="mb-4">
                  <p className="text-sm text-gray-300 mb-2">
                    {selectedRecipe.resultItem.description || "A crafted item."}
                  </p>

                  <div className="space-y-1 text-sm">
                    {selectedRecipe.resultItem.attackBonus > 0 && (
                      <div className="text-orange-400">
                        ATK: +{selectedRecipe.resultItem.attackBonus}
                      </div>
                    )}
                    {selectedRecipe.resultItem.defenseBonus > 0 && (
                      <div className="text-blue-400">
                        DEF: +{selectedRecipe.resultItem.defenseBonus}
                      </div>
                    )}
                    {selectedRecipe.resultItem.healthBonus > 0 && (
                      <div className="text-red-400">
                        HP: +{selectedRecipe.resultItem.healthBonus}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: "0",
                      boxShadow:
                        "0 3px 0 #44403c, inset 0 1px 0 rgba(255,255,255,0.2)",
                  ...
                  <button
                    ...
                    disabled={
                      !hasEnoughMaterials(selectedRecipe) ||
      ...
      {activeTab === "fuse" && (
        <div className="space-y-4">
          <div
            className="bg-stone-900 border-2 border-pink-700 p-3"
            style={{ borderRadius: "0", boxShadow: "0 4px 0 #831843" }}
          >
            <p
              className="text-sm text-pink-200 font-bold"
              style={{ fontFamily: "monospace" }}
            >
              Fuse duplicate companions to evolve them into stronger tiers.
            </p>
            <div
              className="mt-2 grid gap-1 text-xs text-gray-300"
              style={{ fontFamily: "monospace" }}
            >
              <p>• Use 2 copies of the same companion and tier.</p>
              <p>• Each fusion upgrades the tier and boosts stats.</p>
              <p>• Higher tiers give bigger bonuses but cost more gold.</p>
            </div>
          </div>
          ...
          {!fusableCompanions || fusableCompanions.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-400" style={{ fontFamily: "monospace" }}>
                No companions available to fuse
              </p>
              <p
                className="text-xs text-gray-500 mt-2"
                style={{ fontFamily: "monospace" }}
              >
                Get duplicate companions from boss fights!
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {fusableCompanions.map((comp: any) => {
                const canFuse = Object.values(comp.fusableAtTier).some(
                  (v) => v
                );
                const lowestFusableTierEntry = Object.entries(
                  comp.fusableAtTier
                ).find(([_, tierCanFuse]) => tierCanFuse);
                const lowestFusableTier = lowestFusableTierEntry
                  ? Number(lowestFusableTierEntry[0])
                  : 0;
                const copiesAtLowestTier =
                  lowestFusableTierEntry &&
                  comp.tiers[lowestFusableTierEntry[0]]
                    ? comp.tiers[lowestFusableTierEntry[0]].length
                    : 0;
                const goldCost =
                  lowestFusableTier === 0
                    ? 5000
                ...
                const totalCopies = Object.values(comp.tiers).reduce(
                  (sum: number, tierList: any) => sum + tierList.length,
                  0
                );

                // Tier to show in the fusion modal when this card is clicked
                const firstOwnedTierEntry = Object.entries(comp.tiers).find(
                  ([_, tierList]: [string, any]) => tierList.length > 0
                );
                const defaultTierForModal = canFuse
                  ? lowestFusableTier
                  : firstOwnedTierEntry
                  ? Number(firstOwnedTierEntry[0])
                  : 0;

                return (
                  <div
                    key={comp.baseName}
                    className={`bg-stone-900 border-2 ${
                      canFuse
                        ? "border-amber-500 shadow-[0_3px_0_#92400e]"
                        : "border-stone-700 shadow-[0_3px_0_#1c1917]"
                    } hover:-translate-y-[1px] active:translate-y-[2px] active:shadow-none transition-transform`}
                    style={{ borderRadius: "0", fontFamily: "monospace" }}
                    onClick={() => {
                      setSelectedCompanion(comp);
                      setSelectedTier(defaultTierForModal);
                    }}
                  >
                    {/* Card header */}
                    <div className="flex items-stretch border-b border-stone-800 bg-stone-950">
                      <div className="flex items-center justify-center px-3 py-2 border-r border-stone-800 bg-stone-900">
                        <div className="relative w-10 h-10 bg-stone-950 border border-stone-700 flex items-center justify-center">
                          <img
                      ...
                      <div className="flex-1 px-3 py-2">
                        <div className="flex items-center justify-between mb-1">
                          <span
                            className="text-pink-300 font-bold text-sm tracking-wide"
                        ...
                        <div className="text-[10px] text-gray-400">
                          {Object.entries(comp.tiers).map(
                            ([tier, companions]: [string, any]) => {
                              const tierNum = Number(tier);
                              const isTierFusable = comp.fusableAtTier[tierNum];
                              return (
                      ...
                      {canFuse ? (
                        <div className="space-y-1">
                          <p className="text-gray-200">
                            ...
                            →{" "}
                            <span className="text-pink-300 font-bold">
                              T{lowestFusableTier + 1}
                            </span>{" "}
                            · Stats {statMultiplier} · {goldCostDisplay} g
                          </p>
                          <p className="text-gray-400">
                            Requires{" "}
                            <span className="text-pink-200">2 copies</span> at T
                            {lowestFusableTier} ({copiesAtLowestTier}/2 owned).
                          </p>
                          {player && player.gold < goldCost && (
                            <p className="text-red-400">
                              ⚠ Not enough gold to fuse.
                            </p>
                          )}
                        </div>
                      ) : (
                        <p className="text-gray-400">
                          Collect more copies of this companion to unlock
                          fusion.
                        </p>
                      )}
                    </div>
                    ...
                    {/* Card footer */}
                    <div className="px-3 py-1 border-t border-stone-800 bg-stone-950 text-[10px] text-gray-500 flex justify-between">
                      <span>Duplicates owned: {totalCopies}</span>
                      <span>Tap to view fusion details</span>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="text-center mb-4">
              <div className="flex items-center justify-center gap-4 mb-4">
                ...
                <div className="text-center">
                  ...
                  />
                  <span
                    className="inline-block bg-pink-600 text-white px-2 py-1 text-xs font-bold border-2 border-pink-400"
                    style={{ fontFamily: "monospace" }}
                  >
                    T{selectedTier + 1}
                  </span>
