...
ipcMain.on('update-presence', (event, data) => {
  const { details, state, largeImageKey, largeImageText, startTimestamp, endTimestamp } = data;
  updatePresence(details, state, largeImageKey, largeImageText, startTimestamp, endTimestamp);
});
